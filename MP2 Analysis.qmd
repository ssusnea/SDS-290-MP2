...
The largest SD is slightly more than two times (2.12) as big as the smallest for the interaction condition, but is satisfied for condition and car separately, and the graph also does not show a pattern of residuals. Given that this is a really small sample, we expect the condition to be met.
...
```
Given the sum of squares due to regression (SSR) and the sum of squares total (SST) scores from the ANOVA table, the R-squared value is computed to be 0.074. This means that only 7.4% of the variation in college students' willingness of taking the course is explained by the model. 


# Conclusion
This research aims to investigate the correlation between the timing of class offerings and college students' inclination to enroll in the class as well as any interactions between a student's willingness to enroll in a class and their access to a car. We hypothesized that students will be more willing to take an afternoon class over a morning or evening class and that car ownership would affect this willingness as well. Unfortunately, our ANOVA table did not return any evidence to support our hypotheses, so we rejected them all. Our $R^2$ suggests that only $7.4%$ of the variation in our experiment was explained by our model. 

Additionally, several limitations to our research should be considered. Firstly, our sample size was relatively small and largely limited to SDS 290 students which affects our study's generalizability. Secondly, we must consider other confounding variables outside of the scope of our study which likely affect a student's willingness to take an off campus class such as an individual's major. Future research should consider collecting data on these additional variables in order to produce a well rounded understanding of what affects a Smith College student's willingness to take an off campus class. 
